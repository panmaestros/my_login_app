...
Data is sent between the client (ejs template pages) and the server through routes. The routes can then transport data in the form of JSON objects and entire EJS rendered pages between the server and client.

Examples of Client to Server:
...
1. Register a new user on the register page and you should  register successfully and taken to your profile.
...
4. Enter an incorrect password for your account more than 3 times and your account should be locked. To get access back into your account go to the database and set the field `failedloginAttempts` in the `users` table back to 0.
5. To test the brute force prevention limit follow these steps,
  * you should first go stop your server `press ctrl + c`. then go to your `server.js` file in your project.
